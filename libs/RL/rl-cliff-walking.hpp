...
{
public:
    static double goalReward() { return    0; }
    static double stepReward() { return   -1; }
    static double fallReward() { return -100; }
    static double bumpReward() { return Param::stepReward(); }
};
...
private:
    ...
    void step(const action_type a)
    {
        ...
        {
        case Action::actionNorth:
            if (s / CLIFF::length < CLIFF::width - 1)
            { // not upper wall
                s += CLIFF::length;
                r = param.stepReward();
            }
        ...
        case Action::actionSouth:
            ...
            break;
        case Action::actionEast:
            if (s % CLIFF::length < CLIFF::length - 1)
            { // Not on right wall
                r = param.stepReward();
                s++;
            }
