...
protected:
    ...
    q_type q;
    gq_type gq;
    /*virtual*/ void td_update(const STATE& s, const ACTION& a, double td)
    {
        // theta <- theta + alpha*td*grad
        gq(theta, grad, s, a);
        gsl_blas_daxpy(td*alpha, grad, theta);
...
public:
    ...
    /*virtual*/ TD<STATE, ACTION>& operator=(const TD<STATE, ACTION>& cp)
    {
        if (this != &cp)
        {
            if (theta != cp.theta)
    ...
    /*virtual*/ void learn(const STATE& s, const ACTION& a, double r, const STATE& s_, const ACTION& a_)
    {
        this->td_update(s, a, this->td_error(s, a, r, s_, a_));
    }
    /*virtual*/ void learn(const STATE& s, const ACTION& a, double r)
    {
