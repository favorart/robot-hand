...
protected:
    ...
    virtual double nextValue(const STATE& next_state, const ACTION& next_action, unsigned i) const = 0;
...
private:
    ...
    void centralDifferencesTransform()
    {
        ...
        // the 1<= i <= L
        for (size_t i = 1; i < theta_size + 1; ++i)
        {
            this_sigmaPoint = gsl_matrix_column(sigmaPointsSet, i);
            gsl_vector_memcpy(&(this_sigmaPoint.vector), theta);
            gsl_vector_view that_sigmaPoint = gsl_matrix_column(sigmaTheta, i - 1);
            gsl_blas_daxpy(sqrt(theta_size + lambdaUt), &(that_sigmaPoint.vector), &(this_sigmaPoint.vector));
        }
        // the L+1<= i <= 2*L
        for (size_t i = theta_size + 1; i < theta_bound; ++i)
        {
            this_sigmaPoint = gsl_matrix_column(sigmaPointsSet, i);
            gsl_vector_memcpy(&(this_sigmaPoint.vector), theta);
            gsl_vector_view that_sigmaPoint = gsl_matrix_column(sigmaTheta, i - 1 - theta_size);
            gsl_blas_daxpy(-sqrt(theta_size + lambdaUt), &(that_sigmaPoint.vector), &(this_sigmaPoint.vector));
        }
    }
    ...
    void choleskyUpdate(double alpha, gsl_vector *x)
    {
        ...
        */
        ...
        for (unsigned i = 0; i < theta_size; ++i)
        {
            for (unsigned j = 0; j <= i; ++j)
            {
                double tmp = gsl_matrix_get(sigmaTheta, i, j);
        ...
        {
            // diagonal update 
            double tmp = gsl_matrix_get(U, i, i) + gsl_vector_get(x, i)*gsl_vector_get(y, i);
        ...
        for (unsigned i = 0; i < theta_size; ++i)
        {
            double tmp = gsl_matrix_get(U, i, i);
            if (tmp <= 0)
        ...
        for (unsigned i = 0; i < theta_size; ++i)
        {
            ...
            {
                double tmp = gsl_matrix_get(sigmaTheta, i, j) * gsl_vector_get(D, j);
                gsl_matrix_set(sigmaTheta, i, j, tmp);
            }
        }
        // that's all folks !
                      ...
                      bool is_terminal)
    {
        ...
        if (is_terminal)
            for (unsigned i = 0; i < theta_bound; ++i)
            {
                ...
                double qValue = q(&sigmaPoint, state, action);
        ...
        else
        {
            for (unsigned i = 0; i < theta_bound; ++i)
            {
                gsl_vector /* Not a pointer ! */ sigmaPoint;
                sigmaPoint = gsl_matrix_column(sigmaPointsSet, i).vector;
                double qValue = q(&sigmaPoint, state, action);
...
public:
    ...
    KTDQ(const self_type& cp) 
        : super_type(cp), a_begin(cp.a_begin), a_end(cp.a_end)
    {}
