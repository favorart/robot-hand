 ...
 *         ::mt::helpers::random_promotion and
 *         ::mt::helpers::balanced_partition.
 *
...
template <
    typename Data,
    typename DistanceFunction = ::mt::helpers::euclidean_distance,
    typename SplitFunction = ::mt::helpers::split_function<
    ::mt::helpers::random_promotion,
    ::mt::helpers::balanced_partition
    >
>
class mtree
...
public:
    ...
    typedef helpers::cached_distance_function<Data, DistanceFunction> cached_distance_function_type;
...
public:
    ...
    public:
        ...
        query(const mtree* _mtree, const Aim& aim, double range, size_t limit)
            : _mtree(_mtree), aim(aim), range(range), limit(limit)
        {}
        ...
        public:
                ...
                yieldedCount(0)
            {
                ...
                double distance = _query->_mtree->distance_function(_query->aim, _query->_mtree->root->data);
            ...
            void fetchNext()
            {
                ...
                while (!pendingQueue.empty() || !nearestQueue.empty())
                {
                    ...
                    for (typename Node::ChildrenMap::const_iterator i = node->children.begin(); i != node->children.end(); ++i)
                    {
                        ...
                        {
                            double childDistance = _query->_mtree->distance_function(_query->aim, child->data);
     ...
     */
    template <typename Aim>
    typename query<Aim> get_nearest_by_range(const typename Aim& query_data, double range) const
    {
     ...
     */
    template <typename Aim>
    typename query<Aim> get_nearest_by_limit(const typename Aim& query_data, size_t limit) const
    {
     ...
     */
    template <typename Aim>
    typename query<Aim> get_nearest(const typename Aim& query_data, double range, size_t limit) const
    {
        return typename query<Aim>{ this, query_data, range, limit };
    }
     ...
     */
    template <typename Aim>
    typename query<Aim> get_nearest(const typename Aim& query_data) const
    {
        return typename query<Aim>{
            this,
            query_data,
