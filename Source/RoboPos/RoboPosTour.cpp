...
//------------------------------------------------------------------------------
const TourI::JointsNumerator TourI::forward = [](joint_t n_joints, joint_t joint, bool first) -> joint_t {
    if (first) return 0;
    return ((joint == (n_joints - 1)) ? 0 : (joint + 1));
};
const TourI::JointsNumerator TourI::reverse = [](joint_t n_joints, joint_t joint, bool first) -> joint_t {
...
//------------------------------------------------------------------------------
void TourI::appendBreakings(IN muscle_t m)
{
    if (_b_braking)
    {
        joint_t joint = RoboI::jointByMuscle(m);
        ...
        {
            // увеличиваем длительность имеющегося торможения
        ...
        else
        {
            ...
            // добавляем торможение противоположным мускулом
            auto opposite_muscle = RoboI::muscleOpposite(m);
            _breakings_controls[joint] = { opposite_muscle, 0, _lasts_step_braking_init };
...
//------------------------------------------------------------------------------
bool TourI::removeBreakings(IN muscle_t m)
{
    ...
    if (!_b_simul && _b_braking && _breakings_controls_actives > 0)
    {
        ...
        for (joint_t ji = 0; ji <= joint; ++ji, ++it)
        {
            ...
            if (_breakings_controls[ji].muscle != MInvalid && _breakings_controls[ji].lasts > 0)
            {
                ...
                _breakings_controls[ji].start = (_b_simul) ? (it->start) : (it->start + it->lasts + 1);
            }
        }
    }
}
                          ...
                          IN bool /*target_contain !!!*/, IN bool was_on_target)
{
    ...
    if (d > _step_distance /*&& target_contain*/)
    {
        if ((lasts_step / _lasts_step_increment) > 2 || (d / _step_distance) > 2)
        {
            ...
            lasts_step -= 1;
        else //нельзя уменьшить длительность ведущего мускула
        {
            //glob_lasts_step = lasts_step; /// REMOVE
            appendBreakings(control_i.muscle); //подключаем торможение противоположным
        }
    }
    else if (d < _step_distance || !was_on_target)
    {
        /* сначала по возможности отключаем торможения */
        if (!removeBreakings(control_i.muscle))
        {
...
//------------------------------------------------------------------------------
TourI::TourI(RoboMoves::Store *store, Robo::RoboI *robo, const tptree *config, const TourI::JointsNumerator nj) :
    _store(store, [](Store*) {}),
    _robo(robo, [](RoboI*) {}),
    _config(*config),
    _next_joint(nj),
...
void TourI::run()
{
    ...
    try
    {
        ...
        runNestedForMuscle(_next_joint(_robo->jointsCount(), 0/*any*/, true/*first*/), c_useless, useless);
    }
                           ...
                           RoboI::muscleByJoint(joint, false) })
    {
        ...
        for (frames_t last_i = lasts_step; last_i < lasts_i_max; last_i += lasts_step)
        {
            ...
            if (_max_nested)
            {
                ...
                //===============================================================
                runNestedForMuscle(next_joint, c, curr_pos);
                //===============================================================
            }
            else
            {
            ...
            {
                base_pos = curr_pos; // first time only
            }
            //------------------------------------------
                           ...
                           RoboI::muscleByJoint(joint, false) })
    {
        ...
        for (frames_t last_i = (border.min_lasts + lasts_step); (last_i <= border.max_lasts) || (target_contain); last_i += lasts_step)
        {
            ...
            if (_max_nested > 0)
            {
                ...
                auto c = (controls + control_i);
                //===============================================================
                target_contain = runNestedForMuscle(next_joint, c, curr_pos);
                //===============================================================
            }
            else
            {
